...
class ArmController:
    ...
    def move_with_sensor(self, delta_x, delta_y, delta_z, roll, pitch, yaw):
        ...
        target_x = max(self.safety_bounds["x"][0], min(self.initial_position[0] + delta_x, self.safety_bounds["x"][1]))
        target_y = max(self.safety_bounds["y"][0], min(self.initial_position[1] + delta_y, self.safety_bounds["y"][1]))
        target_z = max(self.safety_bounds["z"][0], min(self.initial_position[2] + delta_z, self.safety_bounds["z"][1]))
    ...
    @staticmethod
    def non_linear_scaling(value, scale_factor=1.5):
        return np.sign(value) * (abs(value) ** scale_factor)
