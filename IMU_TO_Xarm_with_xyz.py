...
class ArmController:
    ...
    def move_with_angles(self, roll, pitch, yaw, x, y, z):
        ...
        self.arm.set_servo_cartesian([x, y, z, roll, pitch, yaw], is_radian=False)
...
def process_data_callback(device):
    ...
    x = imu_data.get("AccX", 0) * 1000  # Convert to mm
    y = imu_data.get("AccY", 0) * 1000  # Convert to mm
    z = imu_data.get("AccZ", 0) * 1000  # Convert to mm

    print(f"Moving Arm - Roll: {roll}, Pitch: {pitch}, Yaw: {yaw}, X: {x}, Y: {y}, Z: {z}")
    arm_controller.move_with_angles(roll, pitch, yaw, x, y, z)
